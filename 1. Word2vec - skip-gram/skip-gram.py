...
def make_batch_dataset(word, start, end): 
	...
	for i in range(start, end):
		...
		#논문에서의 R 범위 <1 : window_size> 
		R = np.random.randint(1, window_size+1) 

		#leftset 크기가 R보다 큰경우 R개만 랜덤하게 뽑음.
		if len(leftset) > R:
			p_left = np.arange(1, len(leftset)+1) / np.sum(np.arange(1, len(leftset)+1)) #sum = 1 [1,2,3] => [0.16, 0.33, 0.5]
			leftset = np.random.choice(leftset, R, replace=False, p=p_left) # leftset에서, R개를 랜덤하게 뽑음, 근데 p의 확률 기반으로. replace는 중복 없이 뽑겠다는것.
		
		#rightset 크기가 R보다 큰 경우 R개만 랜덤하게 뽑음.
		if len(rightset) > R:
			p_right = np.arange(len(rightset), 0, -1) / np.sum(np.arange(len(rightset), 0, -1)) # [3, 2, 1] => [0.5, 0.33, 0.16]
			rightset = np.random.choice(rightset, R, replace=False, p=p_right)

		#위 if문에서 leftset이나 rightset이 바뀐경우 : 거리가 가까운것을 높은 확률로, 먼 것을 낮은 확률로 R개 샘플링, 안바뀐경우 : 그대로(R개보다 작음).
		#두 경우 모두 그냥 데이터셋에 넣으면 됨.
		for j in leftset:
			input_.append(word[i])
			target_.append([j])			
...
X = tf.placeholder(tf.int32, (None)) #batch
...
output_of_hidden = tf.nn.embedding_lookup(embedding, X) 
...
nce_loss = tf.nn.nce_loss(weights = nce_weight, biases = nce_bias, labels = Y, inputs = output_of_hidden, num_sampled = negative_sample, num_classes = number_of_word)
